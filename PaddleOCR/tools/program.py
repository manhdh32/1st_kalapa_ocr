    ...
    for epoch in range(start_epoch, epoch_num + 1):
        ...
        for idx, batch in enumerate(train_dataloader):
            ...
            if global_step > start_eval_step and \
                ...
                if len(scores) < 10:
                    scores.append(cur_metric[main_indicator])
                else:
                    min_score = np.min(scores)
                    # logger.info(f"Min score: {min_score}. Cur: {cur_metric[main_indicator]}. Arg min: {np.argmin(scores)}")
                    if cur_metric[main_indicator] > min_score:
                        min_index = np.argmin(scores)
                        scores[min_index] = cur_metric[main_indicator]
                        save_model(
                            model,
                            optimizer,
                            save_model_dir,
                            ...
                            config,
                            is_best=True,
                            prefix=f'step_{global_step}_{cur_metric[main_indicator]}',
                            best_model_dict=best_model_dict,
                            epoch=epoch,
                            global_step=global_step)

...
class RecMetric(object):
    ...
    def __call__(self, pred_label, *args, **kwargs):
        ...
        for (pred, pred_conf), (target, _) in zip(preds, labels):
            wed = Levenshtein.normalized_distance(pred.split(), target.split())
            score += max(0, 1 - pow(1.5, wed)/len(target.split())) if pred.strip() != target.strip() else 1
    ...
    def get_metric(self):
        ...
        score = self.score / (self.all_num + self.eps)
        self.reset()
        return {'acc': acc, 'norm_edit_dis': norm_edit_dis, 'score': score}

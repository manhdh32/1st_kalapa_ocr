...
class AugMultiScaleDataSet(MultiScaleDataSet):
    def resize_norm_img(self, data, imgW, imgH, padding=True):
        img = data['image']

        # random shrink
        h, w, _ = img.shape
        nw = np.random.randint(int(w / 2), w)
        img = cv2.resize(img, (nw, h))

        # random padding
        h, w = img.shape[:2]
        padding_factor = np.random.uniform(1.0, 1.2)
        nh = int(h * padding_factor)
        padding_color = [240, 240, 240]
        y_offset = np.random.randint(0, nh - h) if nh > h else 0
        new_image = np.full((nh, w, 3), padding_color, dtype=np.uint8)
        new_image[y_offset:y_offset + h, 0:w] = img
        img = new_image
        # end #####################
